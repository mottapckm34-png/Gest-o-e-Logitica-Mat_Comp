...
def f_atrativa(lat_navio: float, lon_navio: float,
    ...
    return (magnitude * dlat / norma, magnitude * dlon / norma)


def f_repulsiva(lat_navio: float, lon_navio: float,
                obstaculo: Obstaculo) -> tuple[float, float]:
    # Força que repele o navio dos obstáculos com decaimento exponencial
...
def f_total(lat_navio: float, lon_navio: float,
    ...
    # Soma vetorial de todas as forças atrativas e repulsivas
    fx_total = 0.0
    fy_total = 0.0
